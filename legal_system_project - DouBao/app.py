...
client = Ark(
    ...
    api_key=os.environ.get("1b4bef68-37d5-4196-ba8b-17c9054ae9c5"),
)
...
def chat_legal_knowledge_stream(message, history, request: gr.Request):
    ...
    if not system_state.legal_conversation_history or system_state.current_mode != "legal_knowledge":
        system_state.legal_conversation_history = [{'role': 'system', 'content': prompts.LEGAL_CONSULTANT_PROMPT}]
        system_state.current_mode = "legal_knowledge"
    
    system_state.legal_conversation_history.append({'role': 'user', 'content': message})
    ...
    try:
        ...
        system_state.legal_conversation_history.append({'role': 'assistant', 'content': full_response})
...
def chat_case_strategy_stream(message, history, request: gr.Request):
    ...
    if not system_state.case_conversation_history or system_state.current_mode != "case_strategy":
        system_state.case_conversation_history = [{'role': 'system', 'content': prompts.DEFAULT_LAWYER_SYSTEM_PROMPT}]
    ...
    try:
        if system_state.user_input_round == 1:
            ...
            if detected_type:
                ...
                system_state.case_conversation_history = [{'role': 'system', 'content': system_state.current_lawyer_prompt}]
                
                case_name = config.CASE_TYPES[detected_type]["name"]
                response_msg = f"您好！我是您的专业劳动法律师，很高兴为您提供法律咨询服务。我了解您的案件类型是：{case_name}\n\n现在让我们开始详细了解您的具体情况。"
                
                system_state.case_conversation_history.append({'role': 'user', 'content': message})
                system_state.case_conversation_history.append({'role': 'assistant', 'content': response_msg})
                ...
                system_state.case_conversation_history.append({'role': 'user', 'content': message})
                system_state.case_conversation_history.append({'role': 'assistant', 'content': prompt_response})
        ...
        if system_state.case_type_selected is None:
            ...
            if detected_type:
                ...
                system_state.case_conversation_history.append({'role': 'user', 'content': message})
                system_state.case_conversation_history.append({'role': 'assistant', 'content': selection_prompt})
        ...
        if effective_round <= 4:
            ...
            system_state.case_conversation_history.append({'role': 'user', 'content': polished_message})
        ...
        system_state.case_conversation_history.append({'role': 'assistant', 'content': full_response})
